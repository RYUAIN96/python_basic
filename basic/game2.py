...
types = list( '♠♦♥♣' )
nums  = list( 'A234567689' ) + [ '10' ] + list( 'JQK' )
# 카드 초기화
...
# 2. 카드를 순서대로 나 한장(0), 컴퓨터 한장(1), 나 한장(2), 컴퓨터 한장(3) 배치(순서대로 뽑는다) 전체 2번 반복 = 끝내기
my_cards = cards[:8:2]
my_first_cards = my_cards[:2]
com_cards = cards[1:9:2]
com_first_cards = com_cards[:2]

# 4. 승패 => 내가 가진 카드 중 최대 값 2개를 합산해서, 특별기능이 있다면 추가 계산해서 높은 쪽이 승리한다
# 6. 카드를 받으면 1. 카드를 더 받겠습니까? 아니면 2. 승부를 내겠습니까?
# 플래그 변수 : 변수로 흐름을 제어한다 => cnt
cnt = 0 # 카드를 추가로 준 횟수 
while True:
    choise = input( ' 1. 카드를 더 받겠습니까? 아니면 2. 승부를 내겠습니까? ' ) 

    if choise == '1' and cnt <2:
        cnt += 1
        # 2 -> 3 , 3 -> 4 , 4 -> 1번 선택불가
        my_first_cards = my_cards[:2+cnt]
        com_first_cards = my_cards[:2+cnt]
    elif choise == '2':
        # 판정을 위해 점수를 획득
        myScore  = 0
        comScore = 0
        
        # 5. 한번에 이기면 (내카드의합-컴퓨터카드의합)*100, 카드를 한 번 받으면 20점씩 깎인다
...
# import random # 외부 모듈 가져오기

# ori_data = [ {♥}.format( A, 2 ~ 10, J, Q, K ); , {♦}.format( A, 2 ~ 10, J, Q, K ); , {♠}.format( A, 2 ~ 10, J, Q, K ); , {♣}.format( A, 2 ~ 10, J, Q, K )]
